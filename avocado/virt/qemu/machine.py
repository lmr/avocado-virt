...
class VM(object):
    ...
    def _screendump_thread_start(self):
        self.screendump_dir = utils_path.init_dir(os.path.join(self.logdir,
                                                               'screendumps',
                                                               self.short_id))
        thread_enable = 'avocado.args.virt.screendump_thread_enable'
        self._screendump_thread_enable = self.params.get(thread_enable, True)
        video_enable = 'avocado.args.virt.video_enable'
        self._video_enable = self.params.get(video_enable, True)
    ...
    def _take_screendumps(self):
        ...
        """
        timeout = self.params.get('avocado.args.virt.screendump_thread_timeout',
                                  0.5)
        quality = self.params.get('avocado.args.virt.screendump_thread_quality',
                                  95)
        ...
        while True:
            ...
            if os.path.isfile(ppm_filename):
                if image.is_ppm(ppm_filename):
                    ...
                    img.save(jpg_filename, format="JPEG", quality=quality)
                    s_index += 1
                else:
                    self.log("Produced screendump %s is invalid" % ppm_filename)
                os.unlink(ppm_filename)
    ...
    def _encode_video(self):
        encoder = video.Encoder(verbose=True)
